"""The reason reported when a package cannot be planned.

Range and role both reported "capable and free, but out of the auto-planner's range"
and both suggested passing squadron_id, which the caller may already have passed and
...
class _Squadron:
    """The parts of a Squadron the diagnosis uses, and its two rules: the task has
    to be one it auto-assigns, and the target has to be inside the range limit."""
    ...
    def __init__(
        ...
        # The target measures to the base, as the engine does.
...
class _NeverAssigned(_Squadron):
    """Capable and free but never assigned: a non-helicopter at a FARP, or a
    squadron with no crews left."""
...
def target(monkeypatch: Any) -> Any:
    """Something to measure to. The distance comes from the squadron."""
    aim = SimpleNamespace(
        name="MUSK",
...
def test_a_named_squadron_is_not_told_to_name_a_squadron() -> None:
    """The reported bug: squadron_id was passed and the answer was to pass it."""
...
def test_it_says_how_far_over_the_limit_it_is() -> None:
    """The distance and the limit are what the caller needs to decide what to do."""
    far = _Squadron("Fierce Dragon", away_nm=406)
...
def test_a_role_nobody_auto_assigns_is_not_called_a_range_problem() -> None:
    """The squadron is in range and flies the type; it just does not take the task."""
...
def test_naming_the_squadron_gets_it_the_task_it_does_not_auto_assign() -> None:
    """This is what squadron_id is for, and it still works."""
    near = _Squadron("Fierce Dragon", away_nm=40, roles=set())
    ...
    # That string is the "nothing to report" fallback: the flight is fillable.


def test_a_named_squadron_that_fails_for_another_reason_says_so() -> None:
    """Its task is forced and it is in range, so neither is the cause."""
