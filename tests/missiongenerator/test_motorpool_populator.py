...
def test_cached_motorpool_plan_survives_unchanged_reconciliation() -> None:
    ...
    # The flight plan labels its join and split from the package's flight count
    # (they read NAV when the flight is the whole package), so the stand-in
    # flight needs a package.
    flight = SimpleNamespace(package=SimpleNamespace(flights=[]))
    cached_plan = StrikeFlightPlan(cast(Any, flight), layout)
