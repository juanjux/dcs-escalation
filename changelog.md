...
* **[Mission Generator]** The Hornet's SA page gets its threat rings back, and every cockpit that can draw one gets the front line. DCS 2.9.29 moved the rings behind the aircraft's data cartridge, so a mission with none loaded draws nothing however its units are flagged; Escalation writes one per player airframe that can carry it -- the Hornet family, the Super Hornets and the Viper -- telling it to mirror the mission's own threats, which the MFD settings already filter. Each aircraft carries its cartridge and loads it on spawn, so there is no trip to the DTC page, and a copy goes to Saved Games/DCS/DTC anyway. New setting under MFD, on by default.
...
* **[UI]** The map search filters factories, warehouses, fuel depots and oil separately instead of lumping them under Buildings.
* **[OPFOR-AI]** A building target says which kind of building it is and which base it sits at, and every base reports what its own factories are doing.
...
* **[Live Pilots]** Double-clicking a pilot on a squadron's roster opens his own dialog: his record, what he has survived, how he died, his morale, and what he thinks of the men around him.
...
* **[UI]** An enemy base reports its air defences -- how many sites and of what kind -- alongside its aircraft and ground units.
...
* **[UI]** The base menu says what the base is, who holds it and whether its runway, depots and factory work, with the aircraft, ground units and budget as figures. It can be resized.
...
* **[UI]** The Air Wing Configuration dialog was redrawn: three panes (types, squadrons, bases) instead of a form, squadrons as collapsible cards, and the task grid as chips grouped by family.
...
* **[UI]** A SAM cut off from its network paints its health bar violet, one with no power grey and no rings at all, with the reason in the tooltip.
...
* **[Mission Generation]** New *GPS jamming* option (off by default): a JDAM, JSOW, JASSM or SLAM-ER released inside an enemy jamming bubble lands off the aimpoint, further off the deeper in. Laser and anti-radiation weapons are unaffected (adapted from the 414Ret fork).
...
* **[Campaign]** New campaign "Marianas - Second Island Chain (2027)": a Taiwan crisis gone kinetic, Guam cratered and Rota, Tinian and Saipan taken. No ground front -- the islands change hands by air assault. Brings the China 2027 faction (adapted from the 414Ret fork).
* **[Mission Generation]** Ship groups generate as mixed task groups instead of N copies of one hull: a screen fields destroyers, frigates and a cruiser where the faction has them, capped at three types. The buy menu still gives the hull picked (adapted from the 414Ret fork).
* **[Mission Generation]** Player F/A-18E, F/A-18F and EA-18G spawn with the mission already in the jet -- comm presets, the route, the boat's TACAN/ICLS/ACLS -- like the stock Hornet and Viper. The mod's format has nowhere for an SA picture, so those pages are skipped.
* **[Mission Generation]** The front-line JTAC is the default again for every campaign; the packaged drone JTAC that had replaced it is opt-in per campaign (*Campaign Management -> Insurgency*, off by default, on in the two COIN campaigns). NEW game required.
* **[UI]** **Set as default** on the payload tab: every flight of that airframe and task is generated carrying the fit built there, until *Clear default*. Both sides, every campaign, newly planned flights only; hand-made payloads are never touched.
* **[UI]** Payload tab cleanup: laser-code rows only where the loadout has a use for one, *(customised)* on a changed loadout, the two fuel figures agreeing, a cut store name in full on hover, saving over a name replacing it, and the flight named in the title bar.
* **[Campaigns]** The S-3B Viking is sea control only: its weights out-ranked the A-6E on every land-attack task, so a wing with both sent the anti-submarine aircraft on the bomber's strikes. Strike went back to the A-6E across 23 factions and 39 campaigns. NEW game required.
...
* **[Mission Generation]** Warships fire cruise missile raids: an F10 marker on a shore target ripples a salvo from the nearest capable ship, or auto raids commit one a turn per side. Each ship carries a finite magazine with no rearm. New settings under *Mission Generation -> Naval strike*, both off by default.
...
* **[Mission Generation]** A friendly convoy might be ambushed with no warning: the first sign is the TROOPS IN CONTACT call and an F10 mark when it springs. New setting *Friendly convoy ambushes* (on by default).
* **[Campaign]** The campaign runs on one continuous clock: the time marches a few hours forward per turn, the date rolls over at midnight, and the weather evolves from the turn before instead of being re-rolled. New setting *Continuous time & weather* (on by default).
* **[Campaign Layer]** 1968 Yankee Station's political will has a clock: Washington's patience erodes with the war's duration and Hanoi's resolve breaks under sustained trail interdiction, where both meters used to drift upwards. Hanoi's counter-moves come with a briefing.
* **[Campaigns]** The three Caucasus Vietnam campaigns are one: 1968 Yankee Station carries the whole in-country air war, with the Steel Tiger trail war as an armed-recon tilt and the Khe Sanh siege as the DMZ front. Rebalanced to about 3:1. NEW game required.
* **[Campaigns]** COIN dispersed cells: recon-fogged cells out in the countryside, and one left alone matures and slips home, bringing a destroyed ammo cache back into operation. New setting (off by default, on in Operation Enduring Resolve).
...
* **[Campaigns]** COIN re-infiltration: an under-garrisoned base near a healthy stronghold draws a staged, announced pipeline that flips it back over about four turns, each stage a real unit that can be struck. New setting (off by default, on in Operation Enduring Resolve).
* **[Planner]** New *long-range carrier ops* (off by default): where the carrier stands off beyond the auto-planner's reach, the range gate is raised and one strike package is fragged each turn from the boat's own squadrons, with the wing's other flights tanking from its A-6.
* **[Campaigns]** Enduring Resolve's supply lines follow the real Afghan roads -- Highway 1, Route 611 up the Helmand valley, the Uruzgan mountain road -- instead of straight lines over the ridgelines. Traced with `tools/supply_route_geo.py`.
...
* **[Campaigns]** Enduring Resolve gets the 2006 OEF air war: a multi-national **OEF Coalition 2006** faction (Dutch F-16s, RAF and Marine Harriers, carrier Hornets in the Gulf of Oman, the CENTAF heavies at Kandahar) and a COIN doctrine that lets it strike through gun and IR threat. NEW game required.
* **[Campaigns]** Enduring Resolve draws four positive-control valleys over the real populated river valleys: a fixed strike near the people is priced per kill rather than forbidden. Convoys, troops in contact and air assaults are never gated. NEW game required.
...
* **[Campaigns]** Enduring Resolve's population-center ring has something inside it: FOB Geronimo's caches hide in Lashkar Gah and its supply leg runs through the town, so starving that stronghold prices into the mandate. The empty Herat ring is cut. NEW game required.
...
* **[Campaigns]** New **Afghanistan - Operation Enduring Resolve (COIN)**, a living counterinsurgency (a fork of Starfire's Operation Shattered Dagger): 13 strongholds regenerate unless their hidden caches are struck, and the war is decided by the coalition's mandate against the insurgency's momentum.
* **[Campaign Layer]** The political-will economy is campaign-generic: a campaign's `will:` block re-labels both meters and re-weights every feed, including a new warship-loss feed. The four Vietnam campaigns carry no block and play as before.
...
* **[Campaigns]** New **Nevada - Red Flag 81-2**: the 1981 exercise played as the war it rehearses, against the historically-real Red Force -- Aggressor F-5Es, the 4477th's Constant Peg MiGs at Tonopah, an SA-2/SA-3 emulator array at Tolicha Peak. Two era factions and a three-phase arc.
* **[Mods]** High Digit SAMs support moved from the abandoned original mod to the maintained [Ultimate Compilation](https://github.com/dcs-sams/HighDigitSAMs-Ultimate-Compilation), behind the same New Game toggle, and registers its new content: S-400, S-300V4, Pantsir-SM, SAMP/T, manpads and four EWRs.
* **[Campaigns]** Audited every bundled campaign and swapped aircraft from cut mods for in-game equivalents, honouring each campaign's faction and era (CH B-21 to B-1B, KC-130J to KC-135, F-111C to F-15E, the 1968 carrier wing on F-8E/A-6E/A-1H). Kept mods are untouched.
...
* **[SCAR]** Reworked to loiter-and-task: the flight holds over a static kill box and services a real enemy armor target hidden among decoys, so the kill attrits the enemy the ordinary way. The commander-capture path is inverted -- a SOF team takes him alive. Default on for new campaigns.
* **[Combat SAR]** A new pilot-rescue flight type: a CH-47 orbits near the front while a C-130 flies the HC-130 King orbit with a homing TACAN. Deliver a downed human pilot to a friendly field and the campaign spares that aviator. Player-flown, with an optional AI alert (off by default).
...
* **[Mission Generation]** DEAD and SEAD flights get one waypoint per individual target, with coordinates matching the kneeboard, which gains an STPT column. AI plain-SEAD loiters at a standoff orbit and engages reactively, and AI DEAD flights get the best standoff loadout.
* **[Kneeboard]** New recon kneeboard pages: target reconnaissance, a friendly-packages coordination list and a package-targets theater map. Basemap tiles are cached under Saved Games, with an offline coastline fallback.
...
* **[FIX]** Two convoys on the road could both be called `Convoy 001`: the counter behind the name is not in the save. A campaign that already holds such a pair is repaired on load.
...
* **[FIX]** A patrol in a package (TARCAP, BARCAP, tanker) could be given a takeoff before the mission started. The earliest package TOT is measured against every flight's own takeoff now, rather than estimated.
* **[FIX]** Planning an escort into a package that holds nothing but escorts crashed -- it escorted itself. It flies the package's ordinary geometry now, and a plan that asks for itself raises a planning error.
...
* **[FIX]** Skynet switched an element's radar back on the moment it lost its comms node, powered or not: going autonomous means losing the network, not growing a generator.
* **[FIX]** A tanker planned for a package reached station ninety seconds before an arrival time that is itself an estimate. It holds for the campaign's on-station time and is there ten minutes early.
...
* **[FIX]** Air-defence sites drew a threat ring with nothing at the centre: unticking *Air defences* took the icons away while the threat rings kept drawing. The LORAD/MERAD/SHORAD/AAA rows are filters of that master now, which also stops a site painting two stacked icons.
...
* **[FIX]** The AH-1W carries TOW again in Blufor Late Cold War (80s): DCS models only the 1985 BGM-71D, so a 1983 campaign left the Cobra with rocket pods against armour. The faction overrides the weapon's year to 1970.
...
* **[FIX]** The A-6E's TALDs and its AN/ALQ-167 pod are recognised weapons now: the TALDs were in no weapon file, so they never counted as decoys for SEAD planning and dodged their 1987 date, and the pod never counted as a jammer.
* **[FIX]** **The A-6E drops laser-guided bombs again.** Nothing declared its TRAM turret as a designator, so the planner saw a loadout with no targeting pod and swapped every GBU for iron, in every campaign at every date. The A-6A has no TRAM and is unchanged.
* **[Mission Generation]** Scenery objectives keep their kill-tracking trigger zones even when performance culling skips the rest of the objective: bombing one in a culled region used to destroy a target whose death was never recorded at debrief.
