...
* **[UI]** Command palette: Ctrl+P finds settings, bases, objectives, squadrons, pilots, flights and every command in the menus, and opens what is chosen -- a base or an objective puts the map on it and zooms in, a pilot opens his own dialog.
...
* **[UI]** Buying aircraft: each squadron's row carries its silhouette, task and name, with what is present, its cap, what is on order and what is idle as figures, and a row pinned above the list says what the order leaves behind.
...
* **[UI]** The selected flight plan is drawn whether or not its side's flight plans are switched on in Map Layers.
...
* **[UI]** A waypoint that can be given up — one added by hand, a refuelling stop, the join of a flight that is the whole package — can be deleted on an AI flight again; only *adding* is still reserved for all-player flights.
...
* **[UI]** A squadron can carry its own *Max pilots*, beside *Max size*, instead of every squadron following the campaign-wide limit; it follows the setting until it is moved.
...
* **[Mission Generator]** New *Use JTAC if the faction has one defined* option under Gameplay, beside *Number of JTAC controllers*, so the JTAC aircraft can be switched off mid-campaign instead of only in the new-game wizard.
...
* **[UI]** The Air Wing squadron list was redesigned: the aircraft type is now the one large, bold thing on a fixed rail, to scan for an airframe, the primary task shows as a colour-coded chip, and the list gained a live filter, a sort order, grouping by aircraft type or base, and a running count.
...
* **[Mission Generation]** New *GPS jamming* option (off by default): a JDAM, JSOW, JASSM or SLAM-ER released against a target inside an enemy jamming bubble lands off the aimpoint, further off the deeper in. Laser, TV and anti-radiation weapons are unaffected, and the jammer is an ordinary bombable vehicle (adapted from the 414Ret fork).
* **[OPFOR AI]** The debrief breaks air losses down by type: what died by airframe, what did the killing by weapon, and which airframe killed which.
...
* **[Mission Generation]** Ship groups generate as mixed task groups instead of N copies of one hull: a carrier screen fields destroyers, frigates and a cruiser where the faction has them, capped at three types, and a patrol boat never turns up in a cruiser slot. Naval layouts only; the buy menu still gives exactly the hull picked, and a navy with one hull of the class is unchanged (adapted from the 414Ret fork).
* **[Mission Generation]** Player F/A-18E, F/A-18F and EA-18G spawn with the mission already in the jet -- comm presets, the route with push times and the boat's TACAN/ICLS/ACLS -- the same auto-loading cartridge the stock Hornet and Viper have. The mod's cartridge format has nowhere to put an SA picture, so those sections are skipped rather than written where the jet cannot read them.
* **[Mission Generation]** The front-line JTAC is back and is the default again for every campaign; the packaged drone JTAC that had replaced it fork-wide is opt-in per campaign (*Campaign Management -> Insurgency*, off by default, on in the two COIN campaigns, which is where it is the better model). The drone squadron is only auto-fielded on campaigns that fly it. NEW game required.
* **[UI]** **Set as default** on the payload tab: every flight of that airframe and task planned from then on is generated carrying the fit built there, until *Clear default* hands the slot back. It applies to both sides in every campaign and only to newly planned flights; hand-made Mission Editor payloads are never touched and the file is backed up before the first change.
* **[UI]** Payload tab cleanup: the laser-code rows only appear when the loadout has a use for a code, the Loadout box says *(customised)* instead of reading like the stock preset, the spinner and the fuel-plan line agree, a cut store name shows in full on hover, saving over a name replaces it instead of adding a duplicate, and the window names the flight in its title bar. The weapon laser-code dropdown stays available for AI, which does need a code to drop LGBs on a JTAC's designation.
* **[Campaigns]** The S-3B Viking is sea control only. Its mission weights out-ranked the A-6E Intruder on every land-attack task, so a carrier wing fielding both sent the anti-submarine aircraft on the bomber's strikes. Strike work went back to the A-6E and carrier tanking to the KA-6D across 23 factions and 39 campaigns. NEW game required.
* **[Kneeboards]** Recon kneeboard pages are saved as JPEG instead of PNG, the format DCS's own campaigns use for imagery: a fully-crewed mission drops from 22.4 MB to 9.0 MB with nothing removed. Text pages stay PNG.
* **[Mission Generation]** Every generated mission is archived as well: a named, dated copy lands in `Missions/Retribution Archive/`, which DCS's own mission browser lists, so a past turn re-opens straight from the game. The newest 20 are kept, and the turn is still written to `retribution_nextturn.miz` exactly as before.
* **[Plugins]** Cruise missile auto raids launch staggered inside a random window (240-900 s after mission start, both ends tunable) instead of one mass volley at 240 s, so several launching groups do not stack their salvos against the framerate.
* **[Mission Generation]** Warships fire cruise missile raids. Put an F10 marker on a shore target and call *F10 -> Cruise Missile Strike* to ripple a salvo from the nearest capable ship, or turn on auto raids and each side commits one a turn at its best reachable target, announced to the defender as a bare LAUNCH WARNING. The missiles are real weapons off real, sinkable ships: point defences get to intercept them, and each ship carries a finite campaign magazine with no rearm. New settings under *Mission Generation -> Naval strike*, both off by default.
* **[Settings]** Era-gated cockpit options have their own toggle, *Restrict aircraft options by campaign date* (Difficulty & Realism), instead of riding the weapons restriction -- flip it once if a save relied on that. The gate also covers the A-10C II's Scorpion HMCS and the MiG-29's HMS now, beside JHMCS.
* **[Campaigns]** Nine campaigns had no road between two enemy bases, so the enemy half of the ambient convoys silently never existed there; their real highways are traced now. Every campaign runs at least one side's convoys unless its map genuinely has no two same-side land bases. NEW game required.
* **[Campaigns]** 21 more campaigns got a rear supply corridor traced on their real highways, so 48 of the 67 run convoys; the rest are island or sea-split maps with no road to author. NEW game required.
* **[Mission Generation]** The roads have traffic: every turn each side's supply flow is topped up to a small randomised number of real columns on its own road network, to hunt and to protect. Every column is a tracked transfer whose losses count at debrief. New setting *Ambient supply convoys* (on by default).
* **[Mission Generation]** A friendly convoy might be ambushed, and nothing warns of it beforehand: the first sign is the TROOPS IN CONTACT call and an F10 mark when the ambush springs mid-mission. Both sides are real tracked units -- a ground-down column is reinforcements that never arrive. New setting *Friendly convoy ambushes* (on by default).
* **[Campaign]** The campaign runs on one continuous clock: the mission time marches a few hours forward per turn from the start date, the date rolls over at midnight, and the weather evolves from the turn before instead of being re-rolled, so fronts roll in and clear over several turns. New setting *Continuous time & weather* (on by default, needs day-and-night missions).
* **[Campaign Layer]** 1968 Yankee Station's political will has a clock. Both meters used to drift upwards, so the narrated race had no time pressure: Washington's patience now erodes with the war's duration and Hanoi's resolve can be broken by sustained trail interdiction. Hanoi's counter-moves come with a briefing instead of only being felt.
* **[Campaigns]** The three Caucasus Vietnam campaigns are one. 1968 Yankee Station carries the whole in-country air war -- the Steel Tiger trail war as an armed-recon order-of-battle tilt, the Khe Sanh siege as the DMZ front -- and the two standalone campaigns are dropped. The order of battle was rebalanced to about 3:1 so it plays as a will fight rather than a turkey shoot. NEW game required.
* **[Campaigns]** COIN dispersed cells: the insurgency operates out in the countryside as well as in the strongholds, so there are recon-fogged cells to patrol for. A cell left alone matures and slips into its home stronghold, bringing a destroyed ammo cache back into operation. New setting (off by default, on in Operation Enduring Resolve).
* **[Campaigns]** COIN high-value targets: a named insurgent leader surfaces near a stronghold for a limited strike window, often sheltering among his people, so the shot carries the collateral price the population rings put on it. New setting (off by default, on in Operation Enduring Resolve).
* **[Campaigns]** COIN roadside IEDs: hidden emplacements appear along the insurgent supply roads, to find with TARPS/ISR and strike within a few turns. One left un-swept detonates on the coalition and drains the mandate. New setting (off by default, on in Operation Enduring Resolve).
* **[Campaigns]** COIN re-infiltration: an under-garrisoned base near a healthy stronghold draws a staged, announced pipeline that flips it back over about four turns, each stage a real unit that can be struck. Insurgent bases never outnumber the campaign start -- they relocate, they do not grow. New setting (off by default, on in Operation Enduring Resolve).
* **[Planner]** New *long-range carrier ops* (off by default): where the carrier stands off far beyond the auto-planner's reach, as Enduring Resolve parks it, the range gate is raised and one carrier strike package is fragged each turn from the boat's own squadrons. The wing's other carrier flights tank from that package's A-6, which holds on their own launch and recovery route.
* **[Campaigns]** Enduring Resolve's supply lines follow the roads one would actually drive -- Highway 1 out to Farah, Route 611 up the Helmand valley, the Uruzgan mountain road -- instead of straight lines over the ridgelines. Traced with a new authoring helper, `tools/supply_route_geo.py`; only a route's endpoints bind it to its bases, so the shape in between is free.
* **[Campaigns]** Red Flag 81-2's supply routes are re-traced onto the real NTTR road network, and the worst straight-line legs of the shared Vietnam trail are cleaned up. Every endpoint is untouched, so the routes still bind the same bases.
* **[Campaigns]** Enduring Resolve gets the 2006 OEF air war: a multi-national **OEF Coalition 2006** faction -- Dutch F-16s and RAF Harriers on the Kandahar ramp, Marine Harriers at Bastion, carrier Hornets and an E-2 in the Gulf of Oman, the CENTAF heavies at Kandahar -- and a new COIN doctrine that lets the coalition strike through gun and IR threat, which the suppress-first rule deadlocked against an enemy with no radar SAMs. The insurgents answer with guns at every stronghold and a light radar-SAM crust at the anchors, enough to make SEAD a job again. NEW game required.
* **[Campaigns]** Enduring Resolve draws the horror of COIN onto the map as four positive-control valleys over the real populated river valleys, with the strongholds and much of the insurgency inside them: a fixed strike near the people is priced per kill rather than forbidden. Convoys, troops in contact and air assaults are never gated. NEW game required.
* **[UI]** ROE zones read as shaded areas instead of thin lines on the campaign map (the fill went from 6% to 14%), and Enduring Resolve's four valleys were enlarged to blanket the populated belt.
* **[Campaigns]** Enduring Resolve's population-center ring has something inside it: FOB Geronimo's ammo caches hide in Lashkar Gah and its supply leg runs through the town, so starving that stronghold prices into the mandate while the clean options cost more effort instead. The empty Herat ring is cut -- a ROE ring must guard something. NEW game required.
* **[Campaigns]** Enduring Resolve's ratline runs: the campaign authors eight red supply corridors, the trail picker works where a campaign has no front line, and the rear source is seeded with irregular kit, so there is always a real, interdictable convoy flowing.
* **[Factions]** Toyota Al Gaib 2001 gains the insurgent kit its sibling factions carry: the four DIM' Toyota technicals, the HDS ERO ZU-23 family and the SA-9. Still no armor -- the 2006 insurgency stays light -- and the COIN regeneration pool grows from 6 unit types to 9.
* **[Campaigns]** New **Afghanistan - Operation Enduring Resolve (COIN)**, the first living counterinsurgency campaign (a fork of Starfire's Operation Shattered Dagger). Its 13 strongholds regenerate unless their hidden ammo caches are found and struck, the last recon picture stands until it is re-flown, and the war is decided by the coalition's mandate against the insurgency's momentum over three authored phases. The replenishment engine is generic and off by default.
* **[Campaign Layer]** The political-will economy is campaign-generic: a campaign's `will:` block re-labels both meters and their headlines and re-weights every feed, including a new warship-loss feed, so a naval war bleeds will from sunk ships the way Vietnam bleeds it from downed B-52s. The four Vietnam campaigns carry no block and play exactly as before.
* **[Campaigns]** The Vietnam-era campaigns ship `high_digit_sams: true` and red EWR sites, so the period P-37 Bar Lock their factions gained actually spawns and the red IADS has an early-warning net. HDS Ultimate Compilation is on their Required-Mods lists.
* **[Campaigns]** New **Nevada - Red Flag 81-2**: the 1981 exercise played as the war it rehearses, against the historically-real Red Force -- Aggressor F-5Es on GCI hit-and-run doctrine, the 4477th's Constant Peg MiGs out of Tonopah, an SA-2/SA-3 emulator array around Tolicha Peak. Ships two era factions and a three-phase arc whose Groom Lake box never releases: enter it and the assessment bleeds, exactly like the real range.
* **[Mods]** High Digit SAMs support moved from the abandoned original mod to the maintained [Ultimate Compilation](https://github.com/dcs-sams/HighDigitSAMs-Ultimate-Compilation), behind the same New Game toggle. It absorbs the compilation's breaking changes and registers its new content: S-400 and S-300V4 batteries, the S-300PT launcher, Pantsir-SM, the SAMP/T battery, 70s-80s manpads, four new EWRs including the P-37 Bar Lock, and the ERO ZU-23 technicals.
* **[Campaigns]** Audited every bundled campaign and swapped aircraft from cut mods for in-game equivalents, honouring each campaign's faction and era: CH B-21 to B-1B, CH JAS-39C to F-16CM, the Ukrainian CH types to MiG-29S/Su-24M/Su-27, KC-130J to KC-135, F-111C to F-15E, and the 1968 Yankee Station carrier wing rebuilt on F-8E, A-6E and A-1H. Kept mods are untouched.
* **[Factions]** The same cut-mod swaps applied across the faction files, which the campaign pass had left alone: dedup-aware, carrying each aircraft's livery overrides with it. A new test fails CI if a cut-mod aircraft is re-introduced -- the faction loader otherwise drops it silently, so the regression would be invisible.
...
* **[SCAR]** Strike Coordination and Reconnaissance reworked to loiter-and-task: the flight holds over a static kill box and services a real enemy armor target, still hidden among look-alike decoys, so destroying it attrits the enemy through the ordinary debrief path instead of a bespoke score. The commander-capture path is inverted -- a purchased SOF team assaults the command vehicle left standing and takes him alive -- with a recovery loop for a stranded team. Default on for new campaigns.
* **[Combat SAR]** A new pilot-rescue flight type: a CH-47 orbits near the front as the rescuer while a C-130 flies the HC-130 King on-scene-command orbit, lighting a TACAN every rescue helo can home on. Deliver a downed human pilot to a friendly field and the campaign spares that aviator -- the jet is still lost, the pilot returns to his squadron -- and the same helo can extract a stranded SCAR SOF team. Player-flown, with an optional AI alert (off by default).
* **[Aircraft]** The C-130 fleet is consolidated onto the player-flyable C-130J-30 for transport, SOF airdrop and the Combat SAR King; the stock AI-only C-130 is retired and in-progress campaigns migrate on load. The KC-130 tankers are unchanged.
...
* **[Kneeboard]** New recon kneeboard pages: target reconnaissance, a friendly-packages coordination list and a package-targets theater map. Basemap tiles are fetched once at mission generation and cached under Saved Games, with an offline coastline fallback. Ships a per-terrain airport-imagery dataset, so airfield diagrams line up and the ATIS block shows a temperature-corrected QNH.
...
* **[Cleanup]** Removed DCS: Pretense support entirely -- the one-way exporter, its 590 KB of third-party Lua, the toolbar actions, the settings page, the `Cargo Transport` flight type and the four Pretense-tuned campaigns. Nothing it generated ever came back to the campaign and both sides of the integration are abandoned; an old save reads its cargo flights back as ordinary transports.
* **[Cleanup]** Retired the `dismounts` plugin, a MIST-only, performance-heavy infantry script with no MOOSE successor. First step of the MIST to MOOSE consolidation; old saves drop the orphaned plugin keys on load.
* **[Cleanup]** Retired the `ewrs` plugin, the legacy MIST threat-callout script, superseded by the MOOSE-based `bigeye` EWR.
* **[Cleanup]** Removed the unused `arty` and `artymbot` fire-support scripts, dropped from the active plugin list during the QRA-reserve integration and never re-enabled.
* **[Cleanup]** Deleted dead resource files that still shipped in the build: six orphaned Pretense missions and two legacy root landmap pickles, superseded by the per-theater `theaters/<name>/landmap.p`.
...
## Fixes
* **[FIX]** An escort put on a TARCAP emptied the map -- every route, every icon, and the search with them. The package asked its own escort when to dismiss the escorts, the escort answered by asking the patrol it was protecting, and the patrol answered by asking the package, until the stack ran out and took the request the map draws itself from with it.
...
* **[FIX]** A flight given a TOT offset **ahead** of its package arrived *behind* it instead, so an Escort or SEAD Escort never got there first. The package now leaves room for the flights that have to arrive ahead of it.
...
* **[FIX]** Planning an escort into a package that holds nothing but escorts crashed: the escort was its own package's primary flight, so it escorted itself. It flies the package's ordinary geometry now, and a flight plan that asks for itself raises a planning error naming both flights.
* **[FIX]** A destroyed site was dropped from the IADS network, so its links vanished from the map instead of drawing broken -- and losing the last command centre handed command back. Saves written before this are rebuilt once on load.
...
* **[FIX]** Some building objectives could never be recorded as destroyed: an objective was only credited once every map object in its trigger zone was dead, and many of those zones hold scenery that cannot be destroyed at all. An objective standing on scenery with no damage model is credited on a direct hit now.
* **[FIX]** Faction edits from the Air Wing dialog now take effect in the running campaign.
* **[FIX]** The faction editor's tick boxes never removed anything; entries have a remove button now, which refuses a unit the campaign still has deployed, and its lists are sorted by the name on screen.
* **[Plugins]** The BigEye EWR plugin's *has datalink / RWR / IRST* options never reached the radar-picture engine, so the picture was built from visual and radar alone whatever was ticked.
* **[FIX]** Air-defence sites drew a threat ring with nothing at the centre: unticking *Air defences* in the map layers took the icons and the suspected activity circles away while *Enemy SAM threat range* kept drawing the rings. The LORAD/MERAD/SHORAD/AAA rows are filters of that master now and grey out when it is off, which also stops a site painting two stacked icons when both were ticked.
* **[Campaigns]** Three authored factories never generated: the loader read factory markers from only one of the two authoring blocks, so The Tblisi Gap, Retake the Falklands and Operation Allied Sword each lost one. Every object class now loads from either block. NEW game required.
* **[Aircraft]** Two squadron liveries pointed at personal Saved-Games-only skins and rendered wrong for anyone without them, on a server above all: the UH-1H and the C-130J-30 use in-game liveries now. The C-130J one was the only definition of that transport, so it was wrong in every campaign.
* **[Kneeboard]** Kneeboard content clipped off the right edge: a wide table wraps its over-wide column to the page now, the package FREQ/TOT header line splits when it would overrun, and the threat bullseye lists truncate instead of being cut mid-number.
* **[Kneeboard]** The compact deck's Threats & Targets page left its lower half blank while the enemy air defences were still unidentified; the Fuel Ladder backfills that space when the recon photo has taken the flex page.
* **[FIX]** The AH-1W carries TOW again in Blufor Late Cold War (80s). DCS models only the BGM-71D, dated 1985, so a 1983 campaign left the Cobra with rocket pods against armour -- the AH-1 has carried TOW since 1973. The faction now overrides the weapon's year to 1970, which the weapon file itself suggests as the stand-in for the variants DCS does not model.
* **[FIX]** The AIM-7P is date-gated on the Hornet's LAU-115 rails. Only three of its four clsids were declared, so picking the Sparrow P from those stations gave an unrecognised store with no introduction year -- a 1987 missile that flew in any campaign, however early. The F, M and MH rail clsids were already listed.
* **[FIX]** The A-6E's TALDs and its AN/ALQ-167 pod are recognised weapons now. The Intruder's four MER-mounted TALD stores were in no weapon file, so they read as unknown: they never counted as decoys for SEAD planning, and they dodged the TALD's 1987 date, arming pre-1987 campaigns with an anachronism. The ALQ-167 was missing outright and so never counted as a jammer.
* **[FIX]** **The A-6E drops laser-guided bombs again.** Its TRAM turret is an internal laser designator, but nothing declared it, so the planner saw a loadout with no targeting pod and swapped every GBU for iron -- in every campaign, at every date: `Strike` flew Mk 83 instead of GBU-16, `OCA/Runway` Mk 84 instead of GBU-10, `OCA/Aircraft` Mk 82 instead of GBU-12. The A-6A has no TRAM and is unchanged.
...
* **[Mission Generation]** Fixed an unloadable mission when every preferred neutral nation was already claimed by a belligerent: the generator handed a claimed country to the neutral coalition, putting one nation on two coalitions, which DCS rejects. It picks a genuinely unclaimed nation now.
* **[Plugins]** Restored the **BigEye EWR** plugin to the active plugin list. It was the documented successor to the retired `ewrs` script but had itself been dropped, so the option was missing from the Plugin Options page and there were no early-warning calls at all. Off by default, as before.
* **[Plugins]** Restored the **LotATC export** plugin to the active plugin list, and fixed a cross-wired option in its config: the *Export anti-air symbols* toggle was read into the *Export BLUE anti-air* flag, so symbols could not be turned off and the blue export followed the wrong checkbox.
...
* **[Air Defense]** In-progress campaigns self-heal on load: a stray search radar an older build attached to an AAA site is stripped, so the guns revert to optical and the site stops acting as an EWR node. The KS-19's SON-9 Fire Can is preserved.
...
## Features/Improvements
* **[FIX]** Su-57 payloads no longer lose stations. Its presets asked internal weapons bays for stores they cannot take (an R-73 on station 10 in all seven, rocket pods, a BetAB-500) and asked every anti-ship station for a Kh-59MK2, which this mod declares but offers on no pylon at all — so the anti-ship preset flew with 8 of 12 stations empty. DCS drops such a station silently. Added `tools/audit_mod_pylons.py`, which checks every payload the fork ships against each installed mod's own pylon table.
...
* **[Campaign]** New option "AI buys its better ground units more often" (on by default): the AI picked a ground unit uniformly at random from everything of the right class it could afford, so a faction fielding both a modern MBT and a gun truck bought as many of one as the other. The roll is now weighted by price, the only capability measure the model has -- a weighting, not a maximum, so cheap units still appear (adapted from the 414Ret fork).
* **[Mission Generation]** Two new naval options (both off by default): warships can be released to weapons-free one group at a time across a window instead of every hull opening up at once, and each warship group can carry a finite campaign stock of anti-ship missiles that never rearms — a group that runs dry drops back to return-fire rather than being disarmed. Land-attack cruise missiles keep their own separate magazine, so no shot is charged twice (adapted from the 414Ret fork).
* **[UX]** Every generated mission is also archived to `Missions/Retribution Archive/<campaign>_turn<NN>_<timestamp>.miz`, so `retribution_nextturn.miz` no longer silently overwrites the mission you just flew. Self-pruning, and the fixed output path is unchanged (adapted from the 414Ret fork).
...
* **[IADS]** A destroyed IADS building now reads as destroyed rather than absent: skynet_nodes dropped any node or connection whose units were all dead, and Skynet treats a missing dependency as satisfied, so bombing a power station switched its SAMs back on the next mission and destroying a coalition's last command centre handed it perfect command back. Buildings keep reaching Skynet destroyed (a static spawned dead answers getByName=ok, isExist=false, life=0, which is what Skynet tests); vehicle-backed roles still drop out.
